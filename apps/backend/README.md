...
Production Go backend for DevMind, scaffolded from [sriniously/go-boilerplate](https://github.com/sriniously/go-boilerplate)
per the Phase 7 architecture plan. Echo framework, clean architecture, New Relic observability wired
in from day one.

## Scaffold status (Phase 7, step 1)

This is the boilerplate renamed and pointed at DevMind — nothing domain-specific has been added yet.

**Stock from the boilerplate (works as-is):** config loading, Postgres + Redis wiring, structured
logging with New Relic forwarding, error handling (`errs`/`sqlerr`), the generic `Handle[Req,Res]`
handler wrapper, middleware chain (CORS/rate-limit/recover/request-id/tracing), Asynq job queue,
health checks, tern migrations, testcontainers-based test harness.

**Still stubs, needs DevMind-specific work:** `internal/repository` and `internal/service` (currently
near-empty), auth (Clerk is wired but the GitHub-OAuth-native flow from Phase 7 §5 isn't built), the
AI gateway package (§2), the SSE explanation endpoint (§3/§4), the repo-analysis pipeline (§10), and
the `users`/`analyses`/`cached_explanations` schema (§7) — migration `001_setup.sql` is still empty.

See the roadmap in the DevMind project notes for the build order.
...
```
backend/
├── cmd/devmind/               # Application entry point
...
Configuration is managed through environment variables with the `DEVMIND_` prefix:
